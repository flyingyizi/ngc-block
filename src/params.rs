...
impl Parameter {
    pub fn to_para_index(&self) -> ParaIndex {
        let r = match self {
            &Parameter::CoordSys1(_) => ParaIndex::CoordSys1,
            &Parameter::CoordSys2(_) => ParaIndex::CoordSys2,
            &Parameter::CoordSys3(_) => ParaIndex::CoordSys3,
            &Parameter::CoordSys4(_) => ParaIndex::CoordSys4,
            &Parameter::CoordSys5(_) => ParaIndex::CoordSys5,
            &Parameter::CoordSys6(_) => ParaIndex::CoordSys6,
            &Parameter::CoordSys7(_) => ParaIndex::CoordSys7,
            &Parameter::CoordSys8(_) => ParaIndex::CoordSys8,
            &Parameter::CoordSys9(_) => ParaIndex::CoordSys9,

            &Parameter::ProbePos(_) => ParaIndex::ProbePos,
            &Parameter::ProbeAnalogValue(_) => ParaIndex::ProbeAnalogValue,
            &Parameter::G28Home(_) => ParaIndex::G28Home,
            &Parameter::G30Home(_) => ParaIndex::G30Home,
            &Parameter::G92Offset(_) => ParaIndex::G92Offset,
            &Parameter::CoordSysSelected(_) => ParaIndex::CoordSysSelected,
        };
        r
    }
}
...
type TParamtersMap = BTreeMap<ParaIndex, Parameter>;
...
impl Paramters {
    ...
    pub fn get_coord_sys_selected(&self) -> Option<i32> {
        if let Some(v) = self.0.get(&ParaIndex::CoordSysSelected) {
            match v {
                &Parameter::CoordSysSelected(v) => return Some(v.clone()),
                _ => return None,
            }
        }
        None
    ...
    pub fn get_g92_offset(&self) -> Option<Position> {
        if let Some(v) = self.0.get(&ParaIndex::G92Offset) {
            match v {
                &Parameter::G92Offset(x) => return Some(x.clone()),
                _ => {}
            }
        }
        None
    ...
    pub fn get_sys_coord(&self, orig: i32) -> Option<Position> {
        ...
        if let Some(v) = self.0.get(&index) {
            match v {
                &Parameter::CoordSys1(x) => return Some(x.clone()),
                &Parameter::CoordSys2(x) => return Some(x.clone()),
                &Parameter::CoordSys3(x) => return Some(x.clone()),
                &Parameter::CoordSys4(x) => return Some(x.clone()),
                &Parameter::CoordSys5(x) => return Some(x.clone()),
                &Parameter::CoordSys6(x) => return Some(x.clone()),
                &Parameter::CoordSys7(x) => return Some(x.clone()),
                &Parameter::CoordSys8(x) => return Some(x.clone()),
                &Parameter::CoordSys9(x) => return Some(x.clone()),
                _ => {}
            }
        }
        None
    ...
    pub fn set_sys_coord(&mut self, orig: i32, pos: &Position) -> Result<(), ()> {
        ...
        match orig {
            1 => self.update_item(&Parameter::CoordSys1(pos.clone())),
            2 => self.update_item(&Parameter::CoordSys2(pos.clone())),
            3 => self.update_item(&Parameter::CoordSys3(pos.clone())),
            4 => self.update_item(&Parameter::CoordSys4(pos.clone())),
            5 => self.update_item(&Parameter::CoordSys5(pos.clone())),
            6 => self.update_item(&Parameter::CoordSys6(pos.clone())),
            7 => self.update_item(&Parameter::CoordSys7(pos.clone())),
            8 => self.update_item(&Parameter::CoordSys8(pos.clone())),
            9 => self.update_item(&Parameter::CoordSys9(pos.clone())),
    ...
    pub fn get_field(&self, field_index: FieldIndex) -> Option<f32> {
        ...
        if let Some(p) = self.0.get(&par_index) {
            match p {
                &Parameter::ProbePos(v) => return get_by_index(&v, ofst),
                &Parameter::ProbeAnalogValue(v) => return Some(v),
                &Parameter::G28Home(v) => return get_by_index(&v, ofst),
                &Parameter::G30Home(v) => return get_by_index(&v, ofst),
                &Parameter::G92Offset(v) => return get_by_index(&v, ofst),
                &Parameter::CoordSysSelected(v) => return Some(v as f32),
                &Parameter::CoordSys1(v) => return get_by_index(&v, ofst),
                &Parameter::CoordSys2(v) => return get_by_index(&v, ofst),
                &Parameter::CoordSys3(v) => return get_by_index(&v, ofst),
                &Parameter::CoordSys4(v) => return get_by_index(&v, ofst),
                &Parameter::CoordSys5(v) => return get_by_index(&v, ofst),
                &Parameter::CoordSys6(v) => return get_by_index(&v, ofst),
                &Parameter::CoordSys7(v) => return get_by_index(&v, ofst),
                &Parameter::CoordSys8(v) => return get_by_index(&v, ofst),
                &Parameter::CoordSys9(v) => return get_by_index(&v, ofst),
            }
        }
    ...
    /// update by item
    ///
    pub fn update_item(&mut self, value: &Parameter) {
    ...
    pub fn update_field(&mut self, field_index: FieldIndex, value: f32) {
        ...
        if let Some(p) = self.0.get(&par_index) {
            match p {
                &Parameter::ProbeAnalogValue(v) => {
                    let _ = self.0.insert(par_index, Parameter::ProbeAnalogValue(v));
                }
                &Parameter::CoordSysSelected(v) => {
                    let _ = self.0.insert(par_index, Parameter::CoordSysSelected(v));
                }

                &Parameter::ProbePos(v) => {
                    let xx = replace_by_index(&v, ofst, value);
                    let _ = self.0.insert(par_index, Parameter::ProbePos(xx));
                }
                &Parameter::G28Home(v) => {
                    let xx = replace_by_index(&v, ofst, value);
                    let _ = self.0.insert(par_index, Parameter::G28Home(xx));
                }
                &Parameter::G30Home(v) => {
                    let xx = replace_by_index(&v, ofst, value);
                    let _ = self.0.insert(par_index, Parameter::G30Home(xx));
                }
                &Parameter::G92Offset(v) => {
                    let xx = replace_by_index(&v, ofst, value);
                    let _ = self.0.insert(par_index, Parameter::G92Offset(xx));
                }
                &Parameter::CoordSys1(v) => {
                    let xx = replace_by_index(&v, ofst, value);
                    let _ = self.0.insert(par_index, Parameter::CoordSys1(xx));
                }
                &Parameter::CoordSys2(v) => {
                    let xx = replace_by_index(&v, ofst, value);
                    let _ = self.0.insert(par_index, Parameter::CoordSys2(xx));
                }
                &Parameter::CoordSys3(v) => {
                    let xx = replace_by_index(&v, ofst, value);
                    let _ = self.0.insert(par_index, Parameter::CoordSys3(xx));
                }
                &Parameter::CoordSys4(v) => {
                    let xx = replace_by_index(&v, ofst, value);
                    let _ = self.0.insert(par_index, Parameter::CoordSys4(xx));
                }
                &Parameter::CoordSys5(v) => {
                    let xx = replace_by_index(&v, ofst, value);
                    let _ = self.0.insert(par_index, Parameter::CoordSys5(xx));
                }
                &Parameter::CoordSys6(v) => {
                    let xx = replace_by_index(&v, ofst, value);
                    let _ = self.0.insert(par_index, Parameter::CoordSys6(xx));
                }
                &Parameter::CoordSys7(v) => {
                    let xx = replace_by_index(&v, ofst, value);
                    let _ = self.0.insert(par_index, Parameter::CoordSys7(xx));
                }
                &Parameter::CoordSys8(v) => {
                    let xx = replace_by_index(&v, ofst, value);
                    let _ = self.0.insert(par_index, Parameter::CoordSys8(xx));
                }
                &Parameter::CoordSys9(v) => {
                    let xx = replace_by_index(&v, ofst, value);
                    let _ = self.0.insert(par_index, Parameter::CoordSys9(xx));
                }
            }
        }
    }
...
#[cfg(test)]
mod tests {
    use super::{FieldIndex, Parameter, Paramters, Position};
